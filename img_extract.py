...
#%% Some constants
DATAPATH = 'ImageModule/data/'
OUTPUTPATH = 'ImageModule/generated_output/'
INPUT_XML = sys.argv[1]
MIN_COL_WIDTH = 30  # <- very important! minimum width of a column in pixels, measured in the scanned pages
...
for p_num, p in pages.items():
    ...
    imgfilebasename = INPUT_XML.split('.')[0]+'-1_1'
    print(imgfilebasename)
    imgfile = os.path.join(p['image'])
    print(imgfile)
...
col_positions = get_adjusted_cluster_centers(vertical_lines_clusters, len(vertical_clusters),
...
full_df.to_csv(csv_output_file, index=False, encoding = 'utf-8')
